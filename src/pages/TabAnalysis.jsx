            ...
            <div className="flex flex-col gap-2">
              {getRecommendations(targetType).map((rec, idx) => (
                <button
                  ...
                  className={`btn py-2 text-sm justify-between ${selectedWorkouts.includes(rec.name) ? 'bg-indigo-600 text-white' : 'btn-secondary'}`}
                >
                  <span>{rec.name}</span>
            ...
            <div className="flex justify-around gap-1">
              {Object.keys(routineDays).map(day => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full font-bold text-sm ${routineDays[day] ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-700'}`}
                >
                  {day}
                </button>
              ))}
            ...
            <div className="flex gap-1 justify-between">
              {savedRoutine.days.map(day => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg font-bold text-xs border ${savedRoutine.checked[day] ? 'bg-green-600 border-green-600 text-white' : 'bg-white border-slate-300 text-slate-700'}`}
                >
                  {day} {savedRoutine.checked[day] ? '✅' : '점검'}
                </button>
              ))}
            </div>
